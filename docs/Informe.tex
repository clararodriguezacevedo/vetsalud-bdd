...
En los datos cargados, el campo \code{tipo} distingue principalmente entre \code{Consulta} y \code{Cirugia}; no existe un tipo separado llamado \code{Control}. Por eso se interpreta ``control'' a partir del campo \code{motivo}, seleccionando las consultas cuyo motivo comienza con esa palabra. Al mismo tiempo, se mantiene el filtro \code{tipo = Consulta} para excluir cirugías y otras atenciones clínicas que no correspondan al caso pedido.

El pipeline aplica un \code{\$match} compuesto sobre \code{tipo}, \code{motivo} y \code{costo}. Luego hace un \code{\$lookup} contra \code{pacientes} para mostrar el animal atendido, aplica \code{\$unwind} porque cada consulta tiene un único paciente, ordena por costo ascendente y proyecta los datos principales de la atención.
